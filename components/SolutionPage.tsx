...
const getPlatformCardStyle = (platformId: string): { cardClasses: string; accentColor: string } => {
  switch (platformId) {
    case 'bizboost':
      return { cardClasses: 'border-white/5 hover:border-[#C8377C]/40 hover:shadow-[0_0_40px_-10px_rgba(200,55,124,0.4)]', accentColor: '#C8377C' };
    case 'clarity':
...
const getFeatureCardGlow = (dataId: string): string => {
  ...
  if (dataId === 'tech') return 'border-white/5 hover:border-[#5D2E8E]/40 hover:shadow-[0_0_40px_-10px_rgba(93,46,142,0.35)]';
...
const getAccentDot = (dataId: string): string => {
  ...
  if (dataId === 'tech') return 'bg-[#5D2E8E]';
...
const getHeaderGlow = (dataId: string): string => {
  ...
  if (dataId === 'tech') return '0 20px 60px -15px rgba(93,46,142,0.3)';
  return 'none';
};
...
export const SolutionPage: React.FC<{ data: ServiceData; onBack: () => void; onNavigate?: (page: PageName) => void }> = ({ data, onBack, onNavigate }) => {
  ...
  const handlePlatformClick = (platformId: string) => {
    if (platformId === 'bizboost') {
      window.location.href = `mailto:${LEAD_EMAIL}?subject=Technology%20Services%20Consultation%20(Biz%20Boost)`;
    } else if (onNavigate) {
        ...
        <div className="p-12 lg:p-24 bg-[#0f0f0f]">
          ...
          {data.featuredPlatforms && data.featuredPlatforms.length > 0 && (
            <div className="mt-16 pt-8 border-t border-white/5">
               <h3 className="text-sm font-bold text-gray-400 uppercase tracking-widest mb-6">Our Platforms</h3>
               <div className="space-y-4">
                 {data.featuredPlatforms.map((platform) => {
                   const { cardClasses, accentColor } = getPlatformCardStyle(platform.id);
                   const titleHoverClass = platform.id === 'bizboost' ? 'group-hover:text-[#C8377C]' : platform.id === 'clarity' ? 'group-hover:text-[#06b6d4]' : platform.id === 'championhealth' ? 'group-hover:text-[#2DD4BF]' : 'group-hover:text-[#C5A059]';
                   return (
                     <div
                       key={platform.id}
